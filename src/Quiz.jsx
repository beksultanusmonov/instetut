...
function Quiz() {
  ...
  useEffect(() => {
    ...
    setQuizData(data);
    resetAnswers();
  }, [quizType, navigate, resetAnswers]);
        ...
        <div className="bg-white rounded-xl md:rounded-2xl shadow-lg p-4 md:p-6 mb-6 md:mb-8">
          <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 mb-4">
            ...
            <div className="text-center sm:text-right">
              <div className="text-xl md:text-2xl font-bold text-gray-900">{score} / {quizData.length}</div>
                ...
                {question.answerOptions.map((answerOption, aIndex) => (
                  <div key={aIndex} className="relative">
                    ...
                    >
                      ...
                      <span className="flex-1 text-left text-sm md:text-base">{answerOption.answerText}</span>
                      {answers[qIndex] && answerOption.isCorrect && (
                        <svg className="h-5 w-5 md:h-6 md:w-6 text-green-600 ml-2 shrink-0" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                          <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M5 13l4 4L19 7" />
                        </svg>
                      )}
                      {answers[qIndex] && answers[qIndex].selectedIndex === aIndex && !answerOption.isCorrect && (
                        <svg className="h-5 w-5 md:h-6 md:w-6 text-red-600 ml-2 shrink-0" fill="none" viewBox="0 0 24 24" stroke="currentColor">
              ...
              {answers[qIndex] && !answers[qIndex].isCorrect && (
                <div className="mt-3 md:mt-4 p-3 md:p-4 bg-red-50 border border-red-200 rounded-lg md:rounded-xl px-2 md:px-0 md:ml-14">
                  <div className="flex items-center mb-2">
                    ...
                    <span className="font-semibold text-red-800 text-sm md:text-base">Noto'g'ri javob</span>
                  </div>
                  <p className="text-xs md:text-sm text-gray-700">
                    To'g'ri javob: <span className="font-medium text-green-700">{answers[qIndex].correctAnswer}</span>
                  </p>
                </div>
              )}
            </div>
